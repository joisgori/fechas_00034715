...
function FechaVerif(fecha) {
    var FechaUno = new Date(FechaUno);

    FechaUno = FechaUno.toLocalDateString();
    var FechaDos = FechaDos.split("/");
    var CurrentDate = new Date(Date.now()).toLocaleDateString().split("/");

    if (FechaUno[0] = FechaDos[0]) {
        if (FechaDos[0] < CurrentDate[0]) {
            return 0;
        }
    } else {
        if (FechaDos[1] < CurrentDate[1]) {
            return 0;
        }
    }
    return 1;
